...
// Allow cross-origin texture loads
if (typeof window !== 'undefined') {
    (THREE.TextureLoader.prototype as any).crossOrigin = 'anonymous';
}
...
function GalleryScene({
    ...
    const textures = useTexture(normalizedImages.map((img) => img.src), (loadedTextures) => {
        const arr = Array.isArray(loadedTextures) ? loadedTextures : [loadedTextures];
        arr.forEach((t) => { if (t) t.needsUpdate = true; });
    });
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-black/80">
            <div className="grid grid-cols-3 gap-2 max-h-full overflow-hidden opacity-30">
